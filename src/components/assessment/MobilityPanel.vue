<template>
    <div class="onboarding-panel">
      ...
      <div class="content-wrapper" ref="contentWrapper">
        <div class="panel-content" ref="contentPanel">
          ...
          <h2 class="panel-title">How would you describe your movement right now?</h2>
  
          <v-fade-transition>
            <div class="options-container">
              <div
                v-for="(option, index) in mobilityOptions"
                :key="index"
                @click="selectMobilityOption(option.value)"
                :class="[
                  'mobility-option',
                  'mb-3',
                  { 'selected': selectedMobility === option.value }
                ]"
              >
          ...
          <div class="action-button">
            <app-button block @click="next" :disabled="selectedMobility === null">Continue</app-button>
          </div>
  ...
  const mobilityOptions = [
    { 
      value: 'inactive', 
      label: 'Mostly inactive or resting', 
      icon: 'mdi-bed' 
    },
    { 
      value: 'light', 
      label: 'Light activity with discomfort', 
      icon: 'mdi-walk' 
    },
    { 
      value: 'moderate', 
      label: 'Moderate movement with limitations', 
      icon: 'mdi-run' 
    },
    { 
      value: 'active', 
      label: 'Fully active, but not pain-free', 
      icon: 'mdi-bike' 
    },
  ];
  
  const selectedMobility = ref(null);
  ...
  const selectMobilityOption = (value) => {
    selectedMobility.value = value;
  };
  ...
  const next = () => {
    emit('update', 'mobilityLevel', selectedMobility.value);
    emit('next');
  };
  </script>
